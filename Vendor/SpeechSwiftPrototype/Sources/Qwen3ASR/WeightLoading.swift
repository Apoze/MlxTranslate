...
public enum WeightLoader {
    ...
    /// Layout des poids Conv2d dans le dépôt source (détérminé par la
    /// convention du dépôt, pas par le modèle).
    public enum Conv2dWeightLayout: Sendable {
        /// PyTorch `[outC, inC, kH, kW]` (snapshots aufklarker) — transposé au chargement.
        case pyTorch
        /// MLX `[outC, kH, kW, inC]` (snapshots mlx-community) — utilisé tel quel.
        case mlxNative
        /// Détéction depuis la forme réelle de `conv2d1.weight`.
        case auto
    }
    ...
    public static func loadForcedAlignerWeights(
        ...
        from directory: URL,
        convLayout: Conv2dWeightLayout = .pyTorch
    ) throws {
        let files = try safetensorFiles(in: directory)
        ...
        for file in files {
            ...
            if !audioTowerWeights.isEmpty {
                let transpose = Self.resolveConvTranspose(layout: convLayout, weights: audioTowerWeights)
                applyAudioEncoderComponents(
                    to: model.audioEncoder, weights: audioTowerWeights,
                    transposeConv2dPyTorch: transpose)
    ...
    /// Résout le flag de transposition Conv2d selon le layout demandé et la
    /// forme réelle de `conv2d1.weight` (pour `.auto`).
    ///
    /// - `.pyTorch` → `[outC, inC, kH, kW]` → transposer en `[outC, kH, kW, inC]`.
    /// - `.mlxNative` → `[outC, kH, kW, inC]` → utilisé tel quel.
    /// - `.auto` → détecte : si `conv2d1.weight` a 4 dims et que sa dernière
    ///   dim vaut 1 (in_channels) alors que la 2e ne vaut pas 1, c'est le
    ///   layout MLX natif (pas de transposition). Sinon, layout PyTorch.
    private static func resolveConvTranspose(
        layout: Conv2dWeightLayout,
        ...
        case .mlxNative:
            return false
        case .auto:
            if let probe = weights["conv2d1.weight"],
               probe.ndim == 4, probe.shape.count == 4 {
                let shape = probe.shape
                // MLX natif : [outC, kH, kW, inC] avec inC = 1 pour conv2d1.
                if shape[3] == 1 && shape[1] != 1 {
                    return false
                }
            }
            // PyTorch : [outC, inC, kH, kW] avec inC = 1 pour conv2d1.
            return true
        }
    }
